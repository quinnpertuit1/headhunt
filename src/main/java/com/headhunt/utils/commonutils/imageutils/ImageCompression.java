 ...
 * 
 * @author Rafael Steil
 * @version $Id: ImageCompression.java,v 1.23 2007/09/09 01:05:22 rafaelsteil Exp $
 */
public class ImageCompression {
   ...
   * @return 
   */
  public static File compressImage(String inputFile, String outputFile, int type, int maxWidth, int maxHeight) {
    
    BufferedImage img = resizeImage(inputFile, 0, maxWidth, maxHeight);
    File file = saveCompressedImage(img, outputFile, 0);
   ...
   * @param type The image type. Use <code>ImageCompression.IMAGE_JPEG</code> to save as JPEG images,
   *  or <code>ImageCompression.IMAGE_PNG</code> to save as PNG.
   ...
   * or <code>ImageCompression.IMAGE_PNG</code> to save as PNG.
   */
  public static File saveCompressedImage(BufferedImage image, String toFileName, int type) {
    try {
      if (type == IMAGE_PNG) {
  ...
  {
    if (type == ImageCompression.IMAGE_PNG && hasAlpha(image)) {
  ...
  {
    if (type == ImageCompression.IMAGE_PNG && hasAlpha(source)) {
      type = BufferedImage.TYPE_INT_ARGB;
    }
    else {
      type = BufferedImage.TYPE_INT_RGB;
